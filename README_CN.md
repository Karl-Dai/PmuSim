...
- 🚨 **异常报文跳帧监视面板** — 数据帧时间戳错乱(回退 / 跳变 / 停滞)从事件日志中拆出,在主界面底部独立面板按类型 / 子站筛选、计数徽章、CSV 导出、点行展开详情。
- 🔁 **断线自动重连** — 主站作为客户端外连子站时,每条链路独立指数退避重连,无需手动重连。
- ⏱️ **时间偏差读数** — 实时显示数据帧 SOC/FRACSEC 与本地墙钟的偏差,一眼发现子站对时漂移;并附最大 / 最小时标延迟,在 50 帧滑动窗口内追踪报文时间戳的峰值偏移。
